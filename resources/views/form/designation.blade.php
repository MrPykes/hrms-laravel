...
@section('content')
    ...
    <div class="page-wrapper">
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table table-striped custom-table datatable">
                            ...
                            <tbody>
                                @foreach ($designations as $key=>$items )
                                        <tr class="designation_{{ $items->id }}">
                                            <td hidden class="id">{{ $items->id }}</td>
                                            <td>{{ ++$key }}</td>
                                            <td class="name">{{ $items->name }}</td>
                                            <td class="description">{{$items->description }}</td>
                                            <td class="department" data-id="{{ $items->department_id }}">{{ $items->department->name }}</td>
                                            <td class="text-right">
                                                <div class="dropdown dropdown-action">
                                                    <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a class="dropdown-item edit_designation" data-toggle="modal" data-id="'.$items->id.'" data-target="#edit_designation"><i class="fa fa-pencil m-r-5"></i> Edit</a>
        ...
        <div class="modal custom-modal fade" id="add_designation" role="dialog">
        ...
        <div class="modal custom-modal fade" id="edit_designation" role="dialog">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                ...
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <div class="modal-btn delete-action">
                            <div class="row">
                                <div class="col-6">
                                   <form action="{{ route('designation/destroy')}}" method="POST">
                                        @csrf
    ...
    <script>
        $(document).on('click','.edit_designation',function()
        {
            ...
            let deptId = _this.find('.department').data('id');
            $('#edit_department').val(deptId);
        });
