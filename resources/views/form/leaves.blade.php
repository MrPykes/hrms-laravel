...
@section('content')
                                ...
                                @if(!empty($leaves))
                                    @foreach ($leaves as $items )  
                                        <tr>
                                            ...
                                            <td class="leave_type">{{$items->leave_type->name}}</td>
                                            <td hidden class="from_date">{{ \Carbon\Carbon::parse($items->from_date)->format('d-m-Y') }}</td>
                                            <td>{{\Carbon\Carbon::parse($items->from_date)->format('F d Y')}}</td>
                                            <td hidden class="to_date">{{\Carbon\Carbon::parse($items->to_date)->format('d-m-Y') }}</td>
                                            ...
                                            <td class="text-center">
                                                @php
                                                    ...
                                                    $statusText = ucfirst($items->status); // Pending, Approved, Declined
                                                    $statusColor = match($items->status) {
                                                        'pending'  => 'text-info',
                                                ...
                                                <div class="dropdown action-label">
                                                    <a class="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">
                                                        <i class="fa fa-dot-circle-o {{ $statusColor }}"></i> {{ $statusText }}
                                                    </a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        ...
                                                        <a class="dropdown-item decline" href="javascript:void(0);" 
                                                            data-toggle="modal" 
                                                            data-id="{{ $items->id }}" 
                                                            data-target="#approve_leave">
                                            ...
                                            <td class="text-right">
                                                <div class="dropdown dropdown-action">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a class="dropdown-item leaveUpdate" href="javascript:void(0);" data-toggle="modal" data-id="{{ $items->id }}" data-target="#edit_leave"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                                        <a class="dropdown-item leaveDelete" href="javascript:void(0);" data-toggle="modal" data-id="{{ $items->id }}" data-target="#delete_approve"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        <div class="modal-btn delete-action">
                            <div class="row">
                                ...
                                <div class="col-6">
                                    ...
                                    <form action="{{ route('form/leaves/decline') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="id" id="dc_id" value="">
                                        <button type="submit" class="btn btn-primary continue-btn">Decline</button>
                                    </form>
                                </div>
                            </div>
                        </div>
    ...
    <script>
       ...
       $(document).on('click', '.approve, .decline', function() {
            var _this = $(this).closest('tr'); // safer than parents()
            console.log('aasda',_this.find('.id').text());
            
            $('#a_id').val(_this.find('.id').text());
            $('#dc_id').val(_this.find('.id').text());
        });
